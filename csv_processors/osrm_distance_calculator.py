...
class DistanceCalculatorWorker(QThread):
    ...
    def __init__(self, input_file, port, timeout, delay):
        super().__init__()
    ...
    def build_osrm_url(self, origin_lat, origin_lon, dest_lat, dest_lon):
        try:
            olat = float(origin_lat)
            olon = float(origin_lon)
            dlat = float(dest_lat)
            dlon = float(dest_lon)
            origin_coords = f"{olon:.13f},{olat:.13f}"
            dest_coords = f"{dlon:.13f},{dlat:.13f}"
            return f"http://localhost:{self.port}/table/v1/driving/{origin_coords};{dest_coords}?annotations=distance"
        except (ValueError, TypeError):
            return None

    def get_osrm_data(self, url):
        try:
            response = requests.get(url, timeout=self.timeout)
            response.raise_for_status()
            data = response.json()
            distances = data.get('distances', [[None, None]])
            if len(distances) >= 1 and len(distances[0]) >= 2:
                return distances[0][1]
            return None
        except Exception as e:
            self.progress_updated.emit(0, f"OSRM request failed for URL: {url} - {str(e)}")
            return None

    def process_routes(self, df):
        ...
        for idx, row in df.iterrows():
            ...
            if not url:
                self.progress_updated.emit(0, f"Skipping invalid coordinates at row {idx}")
                continue

            distance = self.get_osrm_data(url)
            df.at[idx, 'URL de Solic.'] = url

            if distance is not None:
                df.at[idx, 'Distance Drive (meters)'] = distance
                processed += 1
                
                # Update progress every 10 rows or when significant progress is made :.2f}%
                ...
                self.progress_updated.emit(0, f"Failed to get distance for row {idx}")
...
class OSRMDistanceCalculator(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("OSRM Distance Calculator")
        self.setGeometry(100, 100, 800, 600)
        
        # Load UI from file
        self.ui = loadUi(self.resource_path('resources/geo_server/osrm_distance_calculator.ui'), self)
    ...
    def set_ui_enabled(self, enabled):
        ...
        self.ui.port_spin.setEnabled(enabled)
